...
/*   Updated: 2019/01/08 23:15:41 by vphongph         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_memjoinfree_b(void *s1, void *s2, size_t l, size_t n)
{
	...
	if ((l && !s1) || (n && !s2) || !(mem = ft_memjoin(s1, s2, l, n)))
	{
		if (ft_putstr_fd_v2(RED"\amemJF_B ∅ pointer | malloc ∅\n"
			RESET, 2) == -1)
			write(2, RED"\amemJF_B -> ∅ pointer | malloc ∅ & putstr fd v2 ∅\n"
				RESET, 75);
		return (NULL);
	}
	free(s1);
